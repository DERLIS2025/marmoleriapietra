...
export default function ProductsPage() {
  return <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-pietra-green">Nuestro catálogo</p><h1 className="mt-3 text-4xl font-bold">Materiales reales para cotizar</h1><p className="mt-4 max-w-2xl leading-8 text-neutral-700">Granitos, cuarzos, mármoles, Neolith y piedra traslúcida disponibles para mesadas, revestimientos, cocinas, baños, quinchos y proyectos especiales.</p><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{allProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div></main>;
}
