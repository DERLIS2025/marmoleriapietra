...
export function ProductCard({ product, compact = false, showRating = false }: ProductCardProps) {
  return (
    <article className="group flex h-full flex-col rounded-[1.35rem] border border-pietra-soft bg-white p-2.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-card">
      <Link href={`/producto/${product.slug}`} className="block">
        <div className="relative overflow-hidden rounded-[1.05rem] bg-pietra-soft">
          ...
          <div className={compact ? "h-24" : "h-40 sm:h-44"}>
      ...
      <div className="flex flex-1 flex-col px-1.5 pt-3">
        ...
        {showRating ? <div className="mt-2 flex text-pietra-beige">{Array.from({ length: 5 }).map((_, index) => <Icon key={index} name="star" className="h-3 w-3" />)}</div> : null}
        <Link href={`/producto/${product.slug}`} className="mt-1.5 line-clamp-2 min-h-[2.75rem] text-sm font-medium leading-snug text-pietra-black transition hover:text-pietra-green sm:text-base">{product.name}</Link>
        <p className="mt-2 line-clamp-2 min-h-[2.5rem] text-xs leading-5 text-neutral-500">{product.shortDescription}</p>
        <div className="mt-auto flex items-center justify-between gap-3 pt-4">
          <span className="text-xs font-medium text-neutral-700">Consultar precio</span>
          <div className="flex gap-1.5">
            ...
            <button aria-label="Agregar a favoritos" className="grid h-9 w-9 place-items-center rounded-full bg-pietra-warm text-pietra-black transition hover:bg-pietra-soft"><Icon name="heart" className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
    </article>
