    ...
    <article className="group rounded-[1.6rem] border border-pietra-soft bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:shadow-soft">
      <Link href={`/producto/${product.slug}`} className="block">
        <div className="relative">
          ...
          <StoneVisual tone={product.tone} label={product.name} className={compact ? "h-28 rounded-[1.1rem]" : "h-48 rounded-[1.25rem]"} />
        </div>
      </Link>
      <div className={compact ? "pt-3" : "px-1 pt-4"}>
        <p className="text-[11px] uppercase tracking-[0.18em] text-pietra-green">{product.category}</p>
        {showRating ? <div className="mt-2 flex text-pietra-beige">{Array.from({ length: product.rating ?? 5 }).map((_, index) => <Icon key={index} name="star" className="h-3.5 w-3.5" />)}</div> : null}
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <a href={createProductWhatsAppLink(product.name)} aria-label={`Consultar por ${product.name}`} className="grid h-9 w-9 place-items-center rounded-full bg-pietra-green text-white transition hover:bg-pietra-black"><Icon name="whatsapp" className="h-4 w-4" /></a>
